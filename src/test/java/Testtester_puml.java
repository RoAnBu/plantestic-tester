...
public class Testtester_puml {
	...
	public Testtester_puml(String configFile) throws Exception {
		try {
	...
	public void test() throws Exception {
		Response roundtrip1 = RestAssured.given()
		        .auth().basic(subst("${SVDS.username}"), subst("${SVDS.password}"))
		    .when()
		        .get(subst("${SVDS.path}") + subst("svds/vehicle/internal/v2/{vin}"))
		    .then()
		        .assertThat()
		...
		Response roundtrip2 = RestAssured.given()
		        .auth().basic(subst("${XCS.username}"), subst("${XCS.password}"))
		    .when()
		        .post(subst("${XCS.path}") + subst("xcs/vin/v1/{vin}"))
		    .then()
		        .assertThat()
		            .statusCode(IsIn.isIn(Arrays.asList(202)))        
		        	.and().extract().response();


		Response roundtrip3 = RestAssured.given()
		        .auth().basic(subst("${CCC.username}"), subst("${CCC.password}"))
		    .when()
		        .get(subst("${CCC.path}") + subst("event/{eventid}"))
		    .then()
		        .assertThat()
		            .statusCode(IsIn.isIn(Arrays.asList(200)))        
		        	.and().extract().response();
		paramsMap.put("test1", roundtrip3.jsonPath().getString("bla"));
	}
